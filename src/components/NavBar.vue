        ...
        <div class="navbar-end">
          ...
          <nav-bar-menu class="has-user-avatar">
            ...
            <div slot="dropdown" class="navbar-dropdown">
              <a class="navbar-item" @click="doChangePassword">
          ...
          <!-- <a class="navbar-item" :title="userName">
            <span>{{ userName }}</span>
          </a> -->
...
export default {
  ...
  methods: {
    ...
    doSave () {
      const self = this
      if (self.frm.current.length > 0 && self.frm.newpass1.length > 0 && self.frm.newpass1 === self.frm.newpass2) {
        Auth.currentAuthenticatedUser()
          .then(user => {
            return Auth.changePassword(user, self.frm.current, self.frm.newpass1)
          })
          .then(data => {
            if (data === 'SUCCESS') {
              ...
              self.modalMessage = data
            }
          })
          .catch(err => {
            self.modalMessage = err.message
          })
      } else {
        self.modalMessage = 'Incorrect Current Password or New Password does not match'
      }
    },
    async logout () {
      const self = this
